...
- **기본 파라미터 파일:** `config/ais_serial.yaml`
...
| `config/ais_serial.yaml` | 기본 노드 파라미터 YAML |
...
| `ais_data_bits` | int `8` | 시리얼 data bit 수. `5`, `6`, `7`, `8` 지원 |
| `ais_parity` | string `"N"` | 시리얼 parity. `N`, `E`, `O`, `M`, `S` 지원 |
| `ais_stop_bits` | double `1` | 시리얼 stop bit 수. `1`, `1.5`, `2` 지원 |
| `ais_xonxoff` | bool `false` | software flow control 사용 여부 |
| `ais_rtscts` | bool `false` | hardware RTS/CTS flow control 사용 여부 |
| `ais_dsrdtr` | bool `false` | DSR/DTR flow control 사용 여부 |
...
| `ais_no_data_warn_sec` | double `10.0` | 포트 open 후 이 시간 동안 바이트가 없으면 경고 로그 출력. `0` 이하면 비활성화 |
| `ais_no_nmea_sync_warn_sec` | double `3.0` | 바이트는 들어오지만 NMEA 시작 문자 `!` 가 안 보일 때 경고 로그 출력. `0` 이하면 비활성화 |
| `ais_debug_hex_dump` | bool `false` | 수신 raw byte preview를 hex dump로 로그 출력 |
| `ais_debug_hex_limit` | int `64` | hex dump에 포함할 최대 byte 수 |
| `ais_debug_publish_reason` | bool `false` | publish되거나 drop되는 이유를 로그 출력 |
| `ais_debug_fragment` | bool `false` | multi-fragment 조립 진행 상황과 만료를 로그 출력 |

### 기본 YAML

`config/ais_serial.yaml`
...
ais_serial_node:
  ros__parameters:
    ...
    ais_no_data_warn_sec: 10.0
    ais_no_nmea_sync_warn_sec: 3.0
    ais_debug_hex_dump: false
    ais_debug_hex_limit: 64
    ais_debug_publish_reason: false
    ais_debug_fragment: false
```

### Launch 인자

`launch/ais_serial.launch.py`에서 직접 override 가능한 주요 launch argument는 다음과 같습니다.

| Launch argument | 기본값 | 설명 |
|---|---:|---|
| `ais_serial_port_name` | `/dev/ttyUSB0` | AIS 시리얼 포트 경로 |
| `ais_baud_rate` | `38400` | AIS 시리얼 보레이트 |
| `ais_data_bits` | `8` | data bit 수 |
| `ais_parity` | `N` | parity (`N`, `E`, `O`, `M`, `S`) |
| `ais_stop_bits` | `1` | stop bit 수 (`1`, `1.5`, `2`) |
| `ais_xonxoff` | `false` | software flow control |
| `ais_rtscts` | `false` | RTS/CTS flow control |
| `ais_dsrdtr` | `false` | DSR/DTR flow control |
| `ais_debug_hex_dump` | `false` | raw byte hex/ascii preview 로그 |
| `ais_debug_publish_reason` | `false` | publish/drop reason 로그 |
| `ais_debug_fragment` | `false` | fragment 조립 로그 |
...
시리얼 포맷이 `minicom`과 다르거나, 바이트는 들어오는데 `!AIVDM` 동기가 안 잡히면 다음처럼 직접 override해서 맞춰볼 수 있습니다.

```bash
ros2 launch aivn_sensorpack_ais ais_serial.launch.py \
  ais_serial_port_name:=/dev/serial/by-id/usb-Actisense_NGX-1_4AE28-if00-port0 \
  ais_baud_rate:=115200 \
  ais_data_bits:=8 \
  ais_parity:=N \
  ais_stop_bits:=1 \
  ais_xonxoff:=false \
  ais_rtscts:=false \
  ais_dsrdtr:=false \
  ais_debug_hex_dump:=true \
  ais_debug_publish_reason:=true \
  ais_debug_fragment:=true
```

노드만 직접 실행해서 확인할 수도 있습니다.
